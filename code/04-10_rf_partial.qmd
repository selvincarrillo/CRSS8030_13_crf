...
## Random forest 
Random forest (has many trees) is a very popular **supervised** “out-of-the-box” or “off-the-shelf” learning algorithm that has good predictive performance with relatively little hyperparameter tuning.  
...
Bagging trees introduces a random component into the tree building process by building **many trees on bootstrapped copies of the training data**.   Note: Takes a random sample of the training data with replacement (bootstrapping) of the same size each time.  
...
Typical default values for mtry are **p/3** for regression (our case) and $\sqrt p$ for classification.  Note: that is the number of columns (features) that we allow the model to see at each split at each tree.

Since the algorithm **randomly selects** a **bootstrap sample** to train on and a **random sample of features to use at each split**, a more diverse set of trees is produced which tends to lessen tree correlation beyond bagged trees and often dramatically increase predictive power. Note: that is, it grabs a random sample of of rows from the data, creates a tree, but only grabbed a random sample of the columns to use at each split.  
...
The number of features or variables selected at each time are the **mtry** hyperparameter. These are random subset of the columns, so each split sees only a subset of the nodes. 
...
library(tidyverse)
library(vip)
library(ranger) # to do random forest
library(finetune)
```
...
Random forest **hyperparameters**:  
  ...
  - **mtry**: number of random features sampled at each node split (columns to select)       
...
rf_spec <- 
  ...
  rand_forest(trees = tune(), # fine tunes the trees for us
              mtry = tune()) %>%
    # Specify the engine
    set_engine("ranger") %>% # relates to the package to use
    # Specifying mode  
...
rf_grid_result <- tune_sim_anneal(object = rf_spec,
                    ...
                    #param_info = rf_param,
                    iter = 50 # at least a 100 for research
                     )
...
rf_grid_result %>%
  ...
  geom_text(aes(label = .iter), nudge_x = .005) +
  labs(title = "RMSE")
```
